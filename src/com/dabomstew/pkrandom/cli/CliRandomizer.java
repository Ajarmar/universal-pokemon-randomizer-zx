...
public class CliRandomizer {
        ...
        try {
            ...
            for (RomHandler.Factory rhf : checkHandlers) {
                if (rhf.isLoadable(romFileHandler.getAbsolutePath())) {
                    ...
                    if (saveLog) {
                        try {
                            randomizer.createDirectory(filename);

                            Integer extStart = filename.indexOf('.');
                            String logFileName = filename.substring(0,extStart);
                            FileOutputStream fos = new FileOutputStream(logFileName + ".html");
                            fos.write(0xEF);
                            fos.write(0xBB);
                            fos.write(0xBF);
                            fos.write(out);
    ...
    private static void printUsage() {
        System.err.println("Usage: java [-Xmx4096M] -jar PokeRandoStar.jar cli -s <path to settings file> " +
